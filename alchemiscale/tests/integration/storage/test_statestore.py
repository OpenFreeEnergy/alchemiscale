...
class TestNeo4jStore(TestStateStore):
    ...
    def test_copy_network_drops_non_complete_extends_ancestor(
        self, n4js, network_tyk2, scope_test
    ):
        """A ``complete`` retry Task that extends an ``error``ed parent
        must not drag the errored parent into the target scope.

        Under the complete-only carry policy, ``update_task_trees``
        gates both the primary Task match and the ``EXTENDS`` parent on
        ``status = complete``. Without the second gate,
        ``to_subgraph`` would clone the errored parent via
        ``get_or_create_task_node(record["extended_task"])``, producing
        an orphan Task in the target scope with no ``PERFORMS`` edge to
        any Transformation.
        """
        source_scope = Scope(**(scope_test.to_dict() | {"project": "extsource"}))
        target_scope = Scope(**(scope_test.to_dict() | {"project": "exttarget"}))
        ...
        # complete retry extending the errored parent
        ...
        # errored parent must NOT appear in target scope
        ...
        # no dangling EXTENDS from the retry in the target scope
        ...
        # errored PDRR must not appear in the target scope; only the ok
        # PDRR from the carried retry does
